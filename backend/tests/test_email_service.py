...
@pytest.mark.asyncio
async def test_send_magic_link_real_mode_includes_recipients(monkeypatch) -> None:
    captured: dict = {}

    async def fake_send(message: str, **kwargs) -> None:
    ...
    assert "Subject: Your LivedHere magic sign-in link" in captured["message"]
